  ...
  load_file().then((xhr) => {
    ...
    data['item'].forEach((item) => {
      ...
      if (b < a) return;
        ...
        value += `<div class="si si1 ${item.subject}${(item.type == 'x' || (now.getTime() > df.getTime() && item.tdis)) ? ' cs' : ''}"><p class="sd">${ds.getFullYear() != now.getFullYear() ? `${ds.getFullYear()}年<br>` : ''}${ds.getMonth() + 1}月${item.type == 'x' ? '' : ds.getDate().toString() + `日${(ds.getTime() != df.getTime() && !item.tdis) ? '-' : ''}`}${item.tdis ? `<br>${ds.getHours()}:${('00' + ds.getMinutes()).slice(-2)}${ds.getTime() == df.getTime() ? '' : '-'}` : ''}</p><p class="st"><span class="sl">${item.contest.join(' ')} | ${item.type == 'c' ? '大会情報' : (item.type == 'a' ? '申し込み情報' : 'その他・未確定情報')}</span><br><span class="nr" style="width:${Math.ceil(Math.max(100, 100 * w / dw)) + 5}%;transform:scalex(${Math.floor(Math.min(100, 100 * dw / w)) * 0.01});">${item.text}</span></p></div>`;
      }
    });
