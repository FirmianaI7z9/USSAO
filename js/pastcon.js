...
function load(index){
  ...
  const contest = document.getElementById('arg').innerText;
  ...
  load_file().then((xhr) => {
    ...
    if (index == 0) {
      value = `<h3>大会ページ</h3><table class="att"><thead><tr><th class="aw10">Contest</th><th class="aw10">Online?</th><th class="aw20">URL</th><th class="aw20">備考</th></tr></thead><tbody>`;
      data['item'].forEach((item) => {
        value += `<tr><th><span class="ico"></span>${contest}${item.year} (#${item.no})</th><td>${item.online ? "オンライン開催" : "現地開催"}</td><td>${item.link != '' ? `<a href="${item.link}"${!item.domestic ? 'target="_blank" rel="noopener noreferrer"' : ''}></a>${item.link}</td>` : ''}<td>${item.info}</td></tr>`
      });
      value += `</tbody></table>`;
    }
    else{
